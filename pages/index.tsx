...
const Index = (): JSX.Element => {
	...
	return (
		<MainLayout title="Home" className="flex-sc col">
			...
			<p className="max-w-sm mb-8 text-center z-10">The template for a quick and intuitive workflow with Next.js and tailwindcss</p>

			<div className="flex-cc gap-4 z-10">
				<Link href="/about" className="px-4 py-2 text-white bg-accent hover:bg-opacity-80">ABOUT</Link>
				<Link href="/form" className="px-4 py-2 text-white bg-black hover:bg-opacity-80">FORM</Link>
			</div>
			...
			<div className="fixed bottom-0 w-full h-1/4 bg-cover flex-cc" style={{backgroundImage: 'url(https://svgshare.com/i/_H4.svg)', paddingBottom: 'env(safe-area-inset-bottom)'}}>
				<p className="flex-cc gap-1 text-lg">See the <Link href="https://github.com/stackoverprof/next-starter" target="_blank" className="font-bold underline">repository</Link></p>
			</div>
		</MainLayout>
	);
};
